...
def split(df, test_size, random_state):
    ...
    print("null samples in train", len(train[train['source'].isna()]), "over", len(train))
    print("null samples in test", len(test[test['source'].isna()]), "over", len(test))

    train, test = train.dropna(), test.dropna()

    print("After null cleaning")
    print("null samples in train", len(train[train['source'].isna()]), "over", len(train))
    print("null samples in test", len(test[test['source'].isna()]), "over", len(test))

    # # drop nan vals
    # if len(train[train['source'].isna()]) or len(test[test['source'].isna()]):
    #     train = train[~train['source'].isna()]
    #     test = test[~test['source'].isna()]
